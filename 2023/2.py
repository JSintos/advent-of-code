...
for string in input:
    ...
    cubeSets = explodedString[1].split(";")

    for cubeSet in cubeSets:
        explodedCubeSet = cubeSet.split(",")

        for numberOfCubes in explodedCubeSet:
            if(numberOfCubes.find("red") != -1):
                if int(''.join(filter(str.isdigit, numberOfCubes))) > 12:
                    isValid = False

            if(numberOfCubes.find("green") != -1):
                if int(''.join(filter(str.isdigit, numberOfCubes))) > 13:
                    isValid = False

            if(numberOfCubes.find("blue") != -1):
                if int(''.join(filter(str.isdigit, numberOfCubes))) > 14:
                    isValid = False

    if isValid:
        explodedGameId = explodedString[0].split(" ")
        
        sum += int(explodedGameId[1])
...
for string in input:
    ...
    for cubeSet in cubeSets:
        ...
        for numberOfCubes in explodedCubeSet:
            amount = int(''.join(filter(str.isdigit, numberOfCubes)))

            if(numberOfCubes.find("red") != -1):
                if highestRed < amount:
                    highestRed = amount

            if(numberOfCubes.find("green") != -1):
                if highestGreen < amount:
                    highestGreen = amount

            if(numberOfCubes.find("blue") != -1):
                if highestBlue < amount:
                    highestBlue = amount
